...
const createTwilioClient = () => {
  const accountSid = process.env.TWILIO_ACCOUNT_SID || 'AC5ef0a194780528e4c4bdeab115cdf2b1';
  const authToken = process.env.TWILIO_AUTH_TOKEN || '990c7b5b95cf2b29b2f5f1c955fa00b8';
  ...
  if (!fromNumber || fromNumber === '+1234567890') {
    console.warn('⚠️ No valid Twilio phone number configured. SMS will be disabled.');
    return null;
  }
  ...
  async testSMSConfiguration(testPhone = '+917812858137') {
    try {
      ...
      if (!twilioConfig) {
        return {
          success: false,
          message: 'SMS service not configured - please set TWILIO_PHONE_NUMBER environment variable with your Twilio number'
        };
      }
